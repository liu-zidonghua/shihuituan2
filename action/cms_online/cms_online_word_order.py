...
class CmsOnlineOrderService(BasePage):
    ...
    """回复工单"""
    def cms_online_op_word_order_reply(self):
        # 点击客服工单
        self.wait_presence_element(CMS_BUTTON_CUSTOMER_SERVICE_WORK).click()
        # 点击工单列表
        self.wait_presence_element(CMS_BUTTON_CUSTOMER_SERVICE_WORK_LIST).click()
        ...
        print( handle2)
        # 　点击工单回复输入框
        self.wait_presence_element(CMS_INPUT_CUSTOMER_SERVICE_WORK_NEWS).click()
        # 　工单回复输入框输入信息
        self.wait_presence_element(CMS_INPUT_CUSTOMER_SERVICE_WORK_NEWS).send_keys("尊敬的团长，客服人员已给您回复，请查看")
        # 　点击回复
        self.wait_presence_element(CMS_BUTTON_CUSTOMER_SERVICE_WORK_NEWS).click()
