...
class TestRegimentalClose():

    @pytest.mark.parametrize('data', regimental_search)
    @allure.step("后台关闭团长验证副团长状态")
    def test_regimental_close(self, cms_web_in, data):
        ...
        with allure.step('7：cms关闭团长'):
            self.driver, self.login_page = (cms_web_in)
            CmsOnlineRegimentalManage(self.driver).cms_online_op_regimental_close()
        ...
        with allure.step('10：cms重起团长'):
            self.driver, self.login_page = (cms_web_in)
            CmsOnlineRegimentalManage(self.driver).cms_online_op_regimental_restart()


if __name__ == '__main__':
    pytest.main(['test_regimentalclose.py'])